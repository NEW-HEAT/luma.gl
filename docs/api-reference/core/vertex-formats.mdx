...
and how it will be exposed to the shader. Each format has a name that encodes
...
luma.gl attempts to provide a consistent API across WebGPU and WebGL, but there are significant differences in vertex formats supported across the two APIs.
...
In general, an argument can be made that in WebGPU / WGSL, attributes are less important
