...
| `createCanvasContext?: CanvasContextProps` \| `true`    | [CanvasContextProps][canvas-context-props]   | Create a default `CanvasContext` for the new `Device`. `true` creates a context with default props.         |
...
| `debugShaders?`: `'errors' 'warnings' 'always' 'never'` | `'errors'`                                   | Display shader source code with inline errors in the canvas.                                                |
...
:::tip
Learn more about GPU debugging in our [Debugging](../../developer-guide/debugging.md) guide.
:::

#### WebGLContextAttributes

For detailed control over the WebGL context, you can specify what [`WebGLContextAttributes`][webgl-attributes] to use if luma creates the WebGL context. 
...
Applications can determine whether the device implements an optional feature by checking `device.features.has(...)`.
...
`loseDevice()` can currently only emulate context loss on WebGL devices on platforms where the WebGL API provides the required `WEBGL_lose_context` WebGL debug extension. 
:::
